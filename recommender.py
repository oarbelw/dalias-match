...
def get_watched_movies(username: str, delay_seconds: float = PAGE_DELAY_SECONDS) -> List[Tuple[str, int]]:
    ...
    page_numbers = [
        int(a.text.strip())
        for a in first_soup.select("div.paginate-pages li a")
...
def build_model_artifacts(df: pd.DataFrame) -> Dict[str, object]:
    ...
    unique_movies = df.drop_duplicates("movie_idx")[
        ["movie_idx", "movie_id", "movie_title", "genres"]
    ].copy()

    genres_for_vector = (
        unique_movies["genres"].fillna("Others").astype(str).str.replace(
            r"\s*,\s*", "|", regex=True
        )
    )
...
def get_similar_users(
    ...
    sim_df = pd.DataFrame(
        {"user_idx": user_item_matrix.index, "similarity": similarities}
    )
